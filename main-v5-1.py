...
def buscarNoEstoque(item_buscado):
    global indice
    ...
    for i in estoque:
        if item_buscado == i[chave]:
            ...
            listarItem(indice) #<<<<---- RESOLVER O PROBLEMA DA LISTAGEM BASEADO NO INDICE PASSADO
'''
def listarItem(i):
    print(f'{i["descricao"]}')'''
...
def selecionaOperacao(nivelPermissao, opcao):
    if nivelPermissao == '1':
        if opcao == 1:
            ...
            item = str(input('Digite a descrição ou codigo do item: '))
            return buscarNoEstoque(item)
        elif opcao == 3:
            return editarItem()
        elif opcao == 4:
            return removerItem()
    ...
    while True:
        if nivel_permissao == '1':
            ...
            if (op < 0 or op > 5):
                print(f'\n -- Opção inválida! --\n')
            if op == 0:
                resp = str(input('\nDESEJA ENCERRAR O SISTEMA? s/n ')).upper()[0] #pega o primeiro caractere
                if resp == 'S':
                    encerrar()
